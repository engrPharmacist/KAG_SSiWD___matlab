...
%-------------------------------------------------------------------------
param.K = 16  ;            % liczba chromosomow K w popoulacji (ze wzgledu na przyjeta strategie powinna byc parzysta)

%param.L = 5;            % dlugosc wektora binarnego L
%param.L = 16;            % dlugosc wektora binarnego L - odwzorowanie liniowe, np. L = 16
param.L = 32*2;            % dlugosc wektora binarnego L - kodowanie binarne liczby zmiennoprzecinkowej pojedynczej precyzji, L = 32

param.fenotyp_max = 5.0;   % minmalna wartosc fenotupu, zdeokdowanego wektora binarnego
param.fenotyp_min = -5.0;  % maksymalna wartosc fenotupu, zdeokdowanego wektora binarnego

param.maxgen = 25;      % maksymalna liczba generacji algorytmu
param.pc = 1;        % prawdopodobienstwo krzyzowania
param.pm = 0.05;        % prawdopodobienstwo mutacji
...
for symulacja = 1:liczba_symulacji
    ...
    for gen = 1:1:param.maxgen
        ...
        % Aktualizuj Rysunek
        xx = param.fenotyp_min:0.1:param.fenotyp_max;      
        [x_1, x_2] = meshgrid(xx, xx);
        y=zeros(size(xx));
        ii = 0;
        jj = 0;
        for i = param.fenotyp_min:0.1:param.fenotyp_max
            ii = ii + 1;
            jj = 0;
            for j = param.fenotyp_min:0.1:param.fenotyp_max
                jj = jj + 1;
                y(ii, jj) = ffun([i; j]);               
            end
        end
        ...
        surfc(x_1,x_2,y);
        hold on
        scatter3(pop(:, param.L+2), pop(:, param.L+1), pop(:, param.L+3),'ro')
        view(-45, 65)
        hold off
